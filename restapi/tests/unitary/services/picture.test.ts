...
function newPictureDto(id: string | undefined,
                       url: string | undefined,
                       place: string | undefined,
                       owner: string | undefined,
                       visibility: Visibility | undefined): PictureDto {
...
describe('PictureService.addOwn() -> ', () => {
    test('picture.url == null', async () => {
        let dtoNull = newPictureDto(generateUUID(), undefined, 'place', 'owner', Visibility.PRIVATE);
        expect(await pictureService.add('sessionId', dtoNull)).toBeFalsy();
    })

    test('picture.place == null', async () => {
        let dtoNull = newPictureDto(generateUUID(), 'text', undefined, 'owner', Visibility.PRIVATE);
        expect(await pictureService.add('sessionId', dtoNull)).toBeFalsy();
    })

    test('picture.visibility == null', async () => {
        let dtoNull = newPictureDto(generateUUID(), 'text', 'place', 'owner', undefined);
        expect(await pictureService.add('sessionId', dtoNull)).toBeFalsy();
    })

    test('picture == OK', async () => {
        let dtoOk = newPictureDto(generateUUID(), 'text', 'place', 'owner', Visibility.PRIVATE);
